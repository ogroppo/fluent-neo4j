...
test('Match node with alias, label, labels, properties, reserved field, set options, remove options', t => {
	...
	t.is(query.queryString, 'MATCH (box:`hey`:`you too`:`Box` {type:{type0}, createdBy:{createdBy1}, createdAt:{createdAt2}}) REMOVE box.ladro, box:`GG`:`SS` SET box.cane = "morto", box:`Bimbo`, box.updatedAt = timestamp() ');
});

test('Match regex on prop', t => {
	const escapeStringRegexp = require('escape-string-regexp')
	var query = new CypherQuery;
	let name = `|!\"£$%&/()=?{}Ç◊ ·—-.,asd><`
	query.matchNode()
	query.where(`node.name =~ '(?i).*${escapeStringRegexp(name)}.*'`)
	t.is(query.queryString, `MATCH (node) WHERE node.name =~ '(?i).*${escapeStringRegexp(name)}.*' `)
});